	...
	def create
		...
		respond_to do |format|
			if @song_listen.save
				...
				if current_user
					Mention.find(2).update_attribute(:remark_id, (Mention.find(2).remark_id + 1)) if params[:s].length == 1 unless current_user.id == 1
					Mention.find(2).update_attribute(:mentionable_id, (Mention.find(2).mentionable_id + 1)) if params[:sh].length == 1 unless current_user.id == 1
				else
